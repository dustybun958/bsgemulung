...
@section('content')
    <style>
        .bg-active {
            ...
            /* Warna hijau mint (sesuai gambar) */
            color: #1C8B69;
            /* Teks warna hijau tua */
        }

        .bg-offline {
            background-color: #FECACA;
            /* Warna merah muda (sesuai gambar) */
            color: #B91C1C;
            /* Teks warna merah tua */
        }
    </style>
                ...
                <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{ route('pedagang.import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="importModalLabel">Import Pedagang dari Excel</h5>
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" required name="id_pedagang"
                                                id="id_pedagang">
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <select name="id_lapak" required id="id_lapak" class="form-control">
                                            <option value="" disabled selected>- Pilih ID Lapak -</option>
                                            <!-- Default option -->
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" required class="form-control" name="nik" id="nik"
                                            maxlength="16" oninput="validateInput(this)">
                                        <div id="nik-list" class="list-group"></div>
                                    </div>
                                    <div class="mb-3">
                                        <div class="mb-3">
                                            <label for="izin" class="form-label">Izin</label>
                                            <input type="text" class="form-control" required name="izin"
                                                id="izin">
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label for="jenis_dagang" class="form-label">Jenis Dagang</label>
                                        <select name="jenis_dagang" id="jenis_dagang" class="form-control">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="id_penarik_retribusi" required id="penarik_retribusi"
                                            class="form-control">
                                            <option value="" disabled selected>- Pilih Penarik -</option>
                                            <!-- Default option -->
                                            @foreach ($penariks as $penarik)
                                                <option value="{{ $penarik->id_penarik_retribusi }}">{{ $penarik->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <tbody class="table-border-bottom-0">
                                    @foreach ($pedagang as $data)
                                        <tr>
                                            ...
                                            <td class="text-start">
                                                <span
                                                    class="badge {{ $data->status == 'Aktif' ? 'bg-active' : 'bg-offline' }} fw-bold p-2"
                                            ...
                                            <td class="text-start">
                                                ...
                                                <div class="align-items-center">
                                                    <a href="{{ route('pedagang.edit', $data->id_pedagang) }}"
                                                        class="btn btn-warning btn-sm">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </a>
                                                    <form id="delete-form-{{ $data->id_pedagang }}"
                                                        action="/admin/pedagang/{{ $data->id_pedagang }}" method="POST"
                                                        class="d-inline-flex align-items-center m-0 p-0 ms-2">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-danger btn-sm"
                                                            onclick="confirmDelete({{ $data->id_pedagang }})">
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function(index, item) {
                                $('#nik-list').append(
                                    '<a href="#" class="list-group-item list-group-item-action nik-item" data-nik="' +
                                    item.nik + '">' + item.nik + ' - ' + item
                                    .nama + '</a>');
                            });
                        } else {
                            $('#nik-list').append(
                                '<a href="#" class="list-group-item list-group-item-action disabled">NIK tidak ditemukan</a>'
                            );
                        }
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function(index, item) {
                                $('#lapak-list').append(
                                    '<a href="#" class="list-group-item list-group-item-action lapak-item" data-id="' +
                                    item.id_lapak + '">' + item.id_lapak +
                                    ' - ' + item.nama_pasar + '</a>');
                            });
                        } else {
                            $('#lapak-list').append(
                                '<a href="#" class="list-group-item list-group-item-action disabled">ID Lapak tidak ditemukan</a>'
                            );
                        }
                    }
                });
